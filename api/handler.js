...
// ─── User storage (serverless-safe) ──────────────────────────────────────────
function getDataFile() {
  if (!fs.existsSync(TMP_DATA)) {
    try {
      const seed = fs.existsSync(BUNDLED_DATA) ? fs.readFileSync(BUNDLED_DATA, "utf8") : "[]";
      fs.writeFileSync(TMP_DATA, seed);
    } catch { fs.writeFileSync(TMP_DATA, "[]"); }
  }
  return TMP_DATA;
}
function loadUsers() {
  try { return JSON.parse(fs.readFileSync(getDataFile(), "utf8")); } catch { return []; }
}
function saveUsers(users) {
  try { fs.writeFileSync(getDataFile(), JSON.stringify(users, null, 2)); } catch {}
}
...
app.get("/api/info", async (req, res) => {
  ...
  try {
    ...
    const data = {
      ...baseData,
      animeInfo: { ...baseData, charactersVoiceActors },
    ...
    const relatedAnimes = (a.relations || []).flatMap(r =>
      (r.entry || []).filter(e => e.type === "anime").map(e => ({
        id: String(e.mal_id), name: e.name, title: e.name, relation: r.relation, poster: "",
      }))
    );
...
async function handleEmbed(req, res, buildUrl) {
  ...
  try {
    ...
    console.error(`[Embed] proxy failed for ${providerUrl}: ${err.message} — redirecting`);
